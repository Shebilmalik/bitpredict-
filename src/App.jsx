import { useState, useEffect, createContext, useContext, useCallback, Fragment } from "react";
import {
    ...
    <div style={{ position:"fixed", top:20, right:20, zIndex:9999, display:"flex", flexDirection:"column", gap:10 }}>
      {list.map(t => (
        <div key={t.id} style={{
          ...
          boxShadow:`0 8px 32px ${t.type==="error"?"#c0392b22":"#27ae6022"}`,
        }}>
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding:"24px 22px 20px", borderBottom:"1px solid #141418" }}>
        ...
        <div style={{ fontSize:10, color:"#3a3a50", marginTop:3, letterSpacing:1 }}>BITCOIN PREDICTION MARKET</div>
      </div>
        ...
        {wallet.isConnected ? (
          <>
            ...
            <span style={{ fontSize:11, color:"#3a3a55", letterSpacing:1 }}>
              {wallet.walletName ? wallet.walletName.toUpperCase() : "WALLET"} · CHAIN {wallet.chainId}
            </span>
          </>
        ) : (
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
        {wallet.isConnected ? (
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            ...
            <button onClick={wallet.disconnect} title="Click to disconnect" style={{
          ...
          <button onClick={wallet.connect} disabled={wallet.connecting} style={{
            ...
            background: wallet.connecting ? "#1a1a25" : "linear-gradient(135deg,#f7931a,#e67e22)",
    ...
    >
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <span style={{
          ...
          {market.cancelled ? "Cancelled" : market.resolved ? (market.outcome ? "✓ YES Won" : "✓ NO Won") : `⏱ ${timeLeft}`}
        </span>
      </div>
      <div style={{ fontSize:14, fontWeight:600, color:"#e0e0f0", lineHeight:1.45 }}>{market.question}</div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:11, color:"#4a4a65", display:"block", marginBottom:6, letterSpacing:1 }}>AMOUNT (BTC)</label>
...
function Markets() {
  ...
  const shown = markets.filter(m => {
    ...
    const statusOk =
      ...
      status === "resolved" ? m.resolved : m.cancelled;
...
function CreateMarket() {
  ...
  const submit = async () => {
    if (!q.trim()) return;
    const secs = DURATIONS.find(d=>d.label===dur)?.secs ?? 604800;
    ...
    <div style={{ maxWidth:580 }}>
      ...
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28 }}>
        ...
        <div style={{ marginBottom:26 }}>
          ...
          <input value={img} onChange={e=>setImg(e.target.value)} placeholder="https://example.com/image.jpg" style={{
            width:"100%", background:"#12121a", border:"1px solid #1e1e2a",
        ...
        <button onClick={submit} disabled={betActions.pending || !q.trim()} style={{
          ...
          cursor: betActions.pending||!q.trim() ? "not-allowed" : "pointer",
              ...
              <div style={{ textAlign:"right", minWidth:110 }}>
                ...
                {claimable(b) && (
                  <button onClick={() => doClaim(b.marketId)} disabled={betActions.pending} style={{
                    ...
                    cursor:"pointer", fontSize:12, fontWeight:700, opacity:betActions.pending ? 0.6 : 1,
                  }}>Claim →</button>
                )}
    ...
    <div>
      ...
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:"20px 22px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:3 }}>
          {["CATEGORY","MARKETS","VOLUME (BTC)","AVG POOL"].map(h => (
            <div key={h} style={{ fontSize:10, color:"#3a3a55", padding:"6px 10px", letterSpacing:1 }}>{h}</div>
          ))}
          {byCat.map(c => (
            <Fragment key={c.name}>
              <div style={{ fontSize:13, color:c.color, padding:"8px 10px" }}>{c.name}</div>
              <div style={{ fontSize:13, color:"#8a8aaa", padding:"8px 10px", fontFamily:"monospace" }}>{c.count}</div>
              <div style={{ fontSize:13, color:"#8a8aaa", padding:"8px 10px", fontFamily:"monospace" }}>{formatBTC(c.vol)}</div>
              <div style={{ fontSize:13, color:"#8a8aaa", padding:"8px 10px", fontFamily:"monospace" }}>
...
function Admin() {
  ...
  const doAddResolver = async () => {
    ...
    try { const tx = await wallet.contract.addResolver(newResolver); await tx.wait(); setNewResolver(""); }
    catch(e) { console.error(e); }
  };

  const Row = ({ label, val, setVal, placeholder, action, btn, color="#f7931a" }) => (
    <div style={{ marginBottom:20 }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        ...
        <button onClick={action} disabled={betActions.pending||!val} style={{
          ...
          opacity:(betActions.pending||!val) ? 0.5 : 1,
        }}>{btn}</button>
      </div>
    </div>
  );
    ...
    <div style={{ maxWidth:600 }}>
      ...
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28, marginBottom:20 }}>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <input value={resolveId} onChange={e=>setResolveId(e.target.value)} placeholder="1" style={{
            width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0",
            padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box",
          }}/>
        </div>
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            {["true","false"].map(v => (
              <button key={v} onClick={()=>setResolveOut(v)} style={{
                flex:1,
                background: resolveOut===v ? (v==="true"?"#2ecc7120":"#e74c3c20") : "#12121a",
        ...
        <button onClick={doResolve} disabled={betActions.pending||!resolveId} style={{
          ...
          opacity:(betActions.pending||!resolveId) ? 0.5 : 1,
        }}>Resolve Market</button>
      </div>

      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28, marginBottom:20 }}>
        <div style={{ fontSize:12, color:"#3a3a50", letterSpacing:1, marginBottom:20 }}>CANCEL MARKET</div>
        <Row label="MARKET ID" val={cancelId} setVal={setCancelId} placeholder="2" action={doCancel} btn="Cancel Market" color="#e74c3c"/>
      </div>

      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28 }}>
        <div style={{ fontSize:12, color:"#3a3a50", letterSpacing:1, marginBottom:20 }}>ADD RESOLVER</div>
        <Row label="ADDRESS" val={newResolver} setVal={setNewResolver} placeholder="0x…" action={doAddResolver} btn="Add" color="#9b59b6"/>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display:"flex", gap:40, padding:"0 24px", alignItems:"center" }}>
        {items.map((t,i) => (
          <span key={i} style={{ fontSize:11, color:"#3a3a55", whiteSpace:"nowrap" }}>{t}</span>
        ))}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const betActions = useBetActions(wallet.contract, wallet.account, (action, hash) => {
    const msgs = { bet:"Bet placed on-chain!", claim:"Reward claimed!", create:"Market created!", resolve:"Market resolved!", cancel:"Market cancelled." };
  ...
  return (
    <Ctx.Provider value={{
      ...
      <div style={{ minHeight:"100vh", background:"#050507", fontFamily:"Inter, sans-serif" }}>
        <Sidebar page={page} setPage={setPage} />
        <div style={{ marginLeft:220, paddingBottom:52 }}>
          <TopBar />
          <main style={{ padding:"28px 32px" }}>
            {loading && markets.length===0 ? (
              <div style={{ textAlign:"center", padding:80, fontSize:13, color:"#3a3a55" }}>
