...
function TopBar() {
  ...
  return (
    <header style={{
      ...
      <div style={{ fontSize:11, color:"#2a2a3a", letterSpacing:1 }}>
        {wallet.chainId ? `CHAIN: ${wallet.chainId}` : "NOT CONNECTED"}
      </div>
      {wallet.isConnected ? (
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:10, color:"#3a3a50" }}>BALANCE</div>
          ...
          <button onClick={wallet.disconnect} style={{
            ...
            color:"#a0a0c0", padding:"8px 14px", borderRadius:8,
        ...
        <button onClick={wallet.connect} disabled={wallet.connecting} style={{
          ...
          opacity: wallet.connecting ? .7 : 1,
        }}>
          {wallet.connecting ? "Connecting…" : "Connect OP Wallet"}
        </button>
      )}
    </header>
  );
...
function BetModal({ market, side, onClose }) {
  const { betActions, refetch } = useCtx();
  const [amount, setAmount] = useState("0.001");
  const payout = calcPotentialPayout(amount, side ? market.yesPool : market.noPool, market.totalPool);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding:28, width:400, maxWidth:"92vw", animation:"fadeUp .2s ease",
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={confirm} disabled={betActions.pending || parseFloat(amount) <= 0} style={{
            ...
            background: side ? "linear-gradient(135deg,#2ecc71,#27ae60)" : "linear-gradient(135deg,#e74c3c,#c0392b)",
...
function Dashboard() {
  ...
  const chartData = markets.slice(-10).map(m => ({ name:`#${m.id}`, volume: parseFloat(m.totalPool) }));

  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      <h1 style={{ fontSize:26, fontWeight:900, color:"#e8e8f0", marginBottom:6 }}>Dashboard</h1>
      <p style={{ fontSize:13, color:"#3a3a55", marginBottom:28 }}>Decentralized prediction markets on Bitcoin L1 · OP_NET</p>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:28 }}>
        {statCards.map(s => (
          <div key={s.label} style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:"18px 22px" }}>
      ...
      {btc && (
        <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:"20px 24px", marginBottom:28, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:10, color:"#3a3a50", letterSpacing:1, marginBottom:6 }}>BITCOIN LIVE PRICE</div>
            <div style={{ fontSize:36, fontWeight:900, color:"#f7931a", fontFamily:"monospace" }}>{fmtUSD(btc.price)}</div>
            <div style={{ fontSize:12, color:"#3a3a55", marginTop:4 }}>Market Cap: {fmtUSD(btc.marketCap)}</div>
          </div>
          <div style={{ fontSize:20, fontWeight:800, color: btc.change24h >= 0 ? "#2ecc71" : "#e74c3c" }}>
...
function Markets() {
  ...
  const shown = markets.filter(m => {
    ...
    const statusOk = status === "all" ? true : status === "active" ? m.isActive : status === "resolved" ? m.resolved : m.cancelled;
...
function CreateMarket() {
  ...
  const submit = async () => {
    ...
    if (hash) { await refetch(); setDone(hash); setQ(""); setImg(""); }
  };
  ...
  return (
    <div style={{ maxWidth:580, animation:"fadeUp .3s ease" }}>
      ...
      {done && (
        <div style={{ background:"#081a0e", border:"1px solid #27ae60", borderRadius:10, padding:"14px 18px", marginBottom:24, fontSize:13, color:"#2ecc71" }}>
          ✓ Market created on-chain!
          <div style={{ fontSize:10, color:"#3a3a55", marginTop:4, fontFamily:"monospace" }}>TX: {done.slice(0,40)}…</div>
          <button onClick={() => setPage("markets")} style={{ marginTop:10, background:"transparent", border:"1px solid #2ecc71", color:"#2ecc71", padding:"5px 14px", borderRadius:6, cursor:"pointer", fontSize:12 }}>View Markets →</button>
        </div>
      )}
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28 }}>
        <div style={{ marginBottom:20 }}>
          <label style={{ fontSize:10, color:"#4a4a65", display:"block", marginBottom:8, letterSpacing:1 }}>QUESTION *</label>
          <textarea value={q} onChange={e=>setQ(e.target.value)} placeholder="Will BTC reach $150,000 before December 2025?" rows={3} style={{ width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"12px 14px", borderRadius:8, fontSize:14, outline:"none", resize:"none", boxSizing:"border-box" }}/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
          <div>
            ...
            <select value={cat} onChange={e=>setCat(e.target.value)} style={{ width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none" }}>
              {Object.entries(CATEGORIES).map(([k,v]) => <option key={k} value={k}>{v.icon} {v.label}</option>)}
            </select>
          </div>
          <div>
            <label style={{ fontSize:10, color:"#4a4a65", display:"block", marginBottom:8, letterSpacing:1 }}>DURATION</label>
            <select value={dur} onChange={e=>setDur(e.target.value)} style={{ width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none" }}>
        ...
        <div style={{ marginBottom:26 }}>
          ...
          <input value={img} onChange={e=>setImg(e.target.value)} placeholder="https://example.com/image.jpg" style={{ width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box" }}/>
        </div>
        {q && (
          <div style={{ background:"#12121a", borderRadius:8, padding:"14px 16px", marginBottom:22 }}>
            ...
            <div style={{ display:"flex", gap:10, marginTop:10 }}>
              <span style={{ fontSize:11, color: CATEGORIES[cat]?.color }}>{CATEGORIES[cat]?.icon} {CATEGORIES[cat]?.label}</span>
              <span style={{ fontSize:11, color:"#3a3a55" }}>· {dur}</span>
            </div>
          </div>
        )}
        {betActions.error && (
          <div style={{ background:"#1a0808", border:"1px solid #c0392b", borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:12, color:"#e74c3c" }}>{betActions.error}</div>
        )}
        <button onClick={submit} disabled={betActions.pending || !q.trim()} style={{
          width:"100%",
          background: betActions.pending || !q.trim() ? "#1a1a25" : "linear-gradient(135deg,#f7931a,#e67e22)",
...
function MyBets() {
  ...
  const claimable = (b) => {
    ...
    return (b.outcome && parseFloat(b.yesAmount) > 0) || (!b.outcome && parseFloat(b.noAmount) > 0);
  };

  const doClaim = async (id) => { await betActions.claimReward(id); refetchBets(); };
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {userBets.map(b => {
          const myYes = parseFloat(b.yesAmount), myNo = parseFloat(b.noAmount);
          const won  = b.resolved && ((b.outcome && myYes>0) || (!b.outcome && myNo>0));
          const lost = b.resolved && !won;
          return (
            <div key={b.marketId} style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:20, display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:10, color: CATEGORIES[b.category]?.color || "#888", marginBottom:6, letterSpacing:1 }}>{CATEGORIES[b.category]?.icon} {b.category?.toUpperCase()}</div>
                <div style={{ fontSize:14, color:"#c8c8e0", lineHeight:1.4, marginBottom:12 }}>{b.question}</div>
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {myYes > 0 && <span style={{ background:"#2ecc7118", color:"#2ecc71", padding:"3px 10px", borderRadius:20, fontSize:11, fontFamily:"monospace" }}>YES: {formatBTC(myYes)} BTC</span>}
                  {myNo  > 0 && <span style={{ background:"#e74c3c18", color:"#e74c3c", padding:"3px 10px", borderRadius:20, fontSize:11, fontFamily:"monospace" }}>NO: {formatBTC(myNo)} BTC</span>}
                  {!b.resolved && !b.cancelled && <span style={{ background:"#f7931a18", color:"#f7931a", padding:"3px 10px", borderRadius:20, fontSize:11 }}>⏱ {formatTimeRemaining(b.endTime)}</span>}
                </div>
              </div>
              <div style={{ textAlign:"right", minWidth:110 }}>
                <div style={{ fontSize:11, fontWeight:700, marginBottom:10, color: won?"#2ecc71":lost?"#e74c3c":b.cancelled?"#9b59b6":"#f7931a" }}>
                  {won?"🏆 Won":lost?"✗ Lost":b.cancelled?"Cancelled":"⏳ Active"}
                </div>
                {claimable(b) && (
                  <button onClick={() => doClaim(b.marketId)} disabled={betActions.pending} style={{ background:"linear-gradient(135deg,#f7931a,#e67e22)", border:"none", color:"#fff", padding:"8px 18px", borderRadius:6, cursor:"pointer", fontSize:12, fontWeight:700, opacity: betActions.pending ? .6 : 1 }}>Claim →</button>
                )}
...
function Analytics() {
  ...
  const byCat = Object.keys(CATEGORIES).map(k => ({
    name: CATEGORIES[k].label, color: CATEGORIES[k].color,
    vol: markets.filter(m=>m.category===k).reduce((s,m)=>s+parseFloat(m.totalPool),0),
    count: markets.filter(m=>m.category===k).length,
  }));
  const sentiment = markets.filter(m=>m.isActive).slice(0,8).map(m => ({ name:`#${m.id}`, YES:m.yesOdds, NO:m.noOdds }));
        ...
        <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:"20px 22px" }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={byCat}>
              ...
              <Tooltip contentStyle={{ background:"#0d0d14", border:"1px solid #1e1e2a", borderRadius:8, fontSize:11 }} />
              <Bar dataKey="vol" radius={[4,4,0,0]}>{byCat.map(entry => <Cell key={entry.name} fill={entry.color} />)}</Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:"20px 22px" }}>
...
function Admin() {
  const { wallet, betActions, refetch } = useCtx();
  ...
  const doResolve = async () => { await betActions.resolveMarket(Number(resolveId), resolveOut==="true"); refetch(); setResolveId(""); };
  const doCancel  = async () => { await betActions.cancelMarket(Number(cancelId)); refetch(); setCancelId(""); };
  const doAddResolver = async () => {
    if (!wallet.contract) return;
    try { const tx = await wallet.contract.addResolver(newResolver); await tx.wait(); setNewResolver(""); } catch(e) { console.error(e); }
  };
    ...
    <div style={{ maxWidth:600, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28, marginBottom:20 }}>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <input value={resolveId} onChange={e=>setResolveId(e.target.value)} placeholder="1" style={{ width:"100%", background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box" }}/>
        </div>
        <div style={{ marginBottom:20 }}>
        ...
        <button onClick={doResolve} disabled={betActions.pending||!resolveId} style={{ width:"100%", background:"linear-gradient(135deg,#3498db,#2980b9)", border:"none", color:"#fff", padding:"12px", borderRadius:8, cursor:"pointer", fontSize:14, fontWeight:700, opacity: betActions.pending||!resolveId ? .5 : 1 }}>Resolve Market</button>
      </div>
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28, marginBottom:20 }}>
        <div style={{ fontSize:12, color:"#3a3a50", letterSpacing:1, marginBottom:20 }}>CANCEL MARKET</div>
        <div style={{ display:"flex", gap:10 }}>
          <input value={cancelId} onChange={e=>setCancelId(e.target.value)} placeholder="Market ID" style={{ flex:1, background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none" }}/>
          <button onClick={doCancel} disabled={betActions.pending||!cancelId} style={{ background:"linear-gradient(135deg,#e74c3c,#c0392b)", border:"none", color:"#fff", padding:"10px 20px", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:700, opacity: betActions.pending||!cancelId ? .5 : 1 }}>Cancel</button>
        </div>
      </div>
      <div style={{ background:"#0a0a10", border:"1px solid #141418", borderRadius:12, padding:28 }}>
        <div style={{ fontSize:12, color:"#3a3a50", letterSpacing:1, marginBottom:20 }}>ADD RESOLVER</div>
        <div style={{ display:"flex", gap:10 }}>
          <input value={newResolver} onChange={e=>setNewResolver(e.target.value)} placeholder="0x…" style={{ flex:1, background:"#12121a", border:"1px solid #1e1e2a", color:"#e8e8f0", padding:"10px 12px", borderRadius:8, fontSize:13, outline:"none" }}/>
          <button onClick={doAddResolver} disabled={!newResolver} style={{ background:"linear-gradient(135deg,#9b59b6,#8e44ad)", border:"none", color:"#fff", padding:"10px 20px", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:700, opacity: !newResolver ? .5 : 1 }}>Add</button>
        </div>
      </div>
      {betActions.error && (
        <div style={{ background:"#1a0808", border:"1px solid #c0392b", borderRadius:8, padding:"10px 14px", marginTop:16, fontSize:12, color:"#e74c3c" }}>{betActions.error}</div>
      )}
    </div>
  );
}
...
function Ticker({ markets }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:0, left:220, right:0, height:36, background:"#07070a", borderTop:"1px solid #141418", display:"flex", alignItems:"center", overflow:"hidden", zIndex:100 }}>
      <div style={{ padding:"0 16px", fontSize:10, color:"#f7931a", fontWeight:700, borderRight:"1px solid #141418", height:"100%", display:"flex", alignItems:"center", whiteSpace:"nowrap" }}>● LIVE</div>
      <div style={{ display:"flex", gap:40, padding:"0 24px", overflow:"hidden", alignItems:"center" }}>
        {items.map((t,i) => <span key={i} style={{ fontSize:11, color:"#3a3a55", whiteSpace:"nowrap" }}>{t}</span>)}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const betActions = useBetActions(wallet.contract, wallet.account, (action, hash) => {
    const msgs = { bet:"Bet placed on-chain!", claim:"Reward claimed!", create:"Market created on-chain!", resolve:"Market resolved!", cancel:"Market cancelled." };
  ...
  return (
    <Ctx.Provider value={{ wallet, markets, totalVolume, loading, refetch, userBets, ubLoading, refetchBets, betActions, onBet, setPage }}>
      <div style={{ minHeight:"100vh", background:"#050507" }}>
          ...
          <main style={{ padding:"28px 32px" }}>
            {loading && markets.length===0 ? (
              <div style={{ textAlign:"center", padding:80 }}>
                <div style={{ fontSize:13, color:"#3a3a55" }}>Loading markets from chain…</div>
              </div>
            ) : (
              <PageComp />
            )}
        ...
        {betModal && <BetModal market={betModal.market} side={betModal.side} onClose={() => setBetModal(null)} />}
        <Toasts list={toasts} />
      </div>
    </Ctx.Provider>
  );
